...
| [gala](pkgs/gala/) | [series](pkgs/gala/series) | <ul><li>Stop crash loop from unbalanced `WorkspaceManager.thaw_remove` when a workspace is added outside a swipe gesture</li><li>Prevent focus-stealing on attention demand</li><li>Never let notification bubbles hold keyboard focus on X11 (mutter's focus-on-map races the daemon's user-time hint — bounce focus back to the MRU window)</li><li>Stop the crash when dismissing a notification bubble. Upstream 8.6.1's use-after-free guard only fires for `NOTIFICATION`-type windows, but `NotificationsClient` rewrites bubbles to `DOCK`, so `destroy_completed` still ran on a freed actor</li></ul> |
