...
`ep restart <package>` sends SIGTERM to the package's systemd user unit and waits for a new `MainPID`. Only `gala` has a mapping (`io.elementary.gala@<session type>.service`). Don't use `gala --replace`: it starts a gala *outside* the unit, the unit's `Restart=always` starts another one at once, the two fight, and after `StartLimitBurst=3` in 15s systemd fails the session. `RefuseManualStop=on` blocks `systemctl --user stop|restart`, but not `systemctl --user kill`, which is what the script uses. It refuses to run within 15s of the unit's last start so a restart never spends the second of the three allowed starts. On X11 the windows survive — they're X clients, gala re-manages them.
